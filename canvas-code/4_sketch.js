...
function buttonCallback(value) {
  oldSliderValue = value.data;
  slider.value(value.data);              
}

function motionCallback(value) {
  var color = value.data.split(",")
  console.log(color);
  r = parseInt(color[0]);
  g = parseInt(color[1]);
  b = parseInt(color[2]);    
      ...
      function(devices){
        for (var i = 0; i < devices.length; i++) {
          ...
          if (currentDevice.connected) {
            ...
            photonDevice.onEvent('level', buttonCallback);
            photonDevice.onEvent('motion', motionCallback);
